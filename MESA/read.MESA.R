## Read in MESA files
## NOTE: throws a warning of "EOF within quoted string" at i = 64
## [64] "phs000209.v11.pht003091.v1.p3.c1.MESA_Exam5Main.HMB.txt"
## Also - this script takes ~1 hour to run, so be aware of that!

setwd("~/Desktop/work/2015-2018/HMS DBMI/MESA")
library(XML)

## Create list of all text and XML data files in MESA cohort
...
## Convert all text and XML data files to data frame format
...
for (i in 1:num_files) {
      ...
      ## Convert variables and children to data frames, export to CSV file(s).
      child_name = c()
      child_desc = c()
      child_value = c()
      for (j in variables_start:variables_end) {
            ...
            ## export to CSV file (type is ignored)
            ...
            ## Format extra children to data frame(s), export to CSV file(s).
            num = j - variables_start + 1
            name2 = rep(name[num], extra_children)
            
            ## Parse out attributes for encoded and/or numeric values
