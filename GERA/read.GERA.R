## Read in GERA files
setwd("~/Desktop/work/2015-2018/HMS DBMI/GERA")
library(XML)

## Create list of all text and XML data files in GERA cohort
...
for (i in 1:num_files) {
      ...
      for (j in variables_start:variables_end) {
            ...
            if (length(node_codes) != 0) {
                  value_temp = xpathSApply(xml_node, "//value", xmlAttrs)
                  if (is.null(value_temp[[1]]) == TRUE) {
                        next
                  }
